...
//$tickers=array("EURUSD=X","GBPUSD=X","EURGBP=X","USDCHF=X","EURJPY=X","USDJPY=X","USDCAD=X","AUDUSD=X");
$start=microtime();
$str="'".implode("','", $forex_tickers)."'";
$columns="snl1b3b2bad1t1m2w";
$file="http://download.finance.yahoo.com/d/quotes.csv?f=".$columns."&s=".$str;

$stocks=array();
$handle = fopen($file, "r");
$sql="INSERT INTO forex_temp (symbol, name, last_trade, bid_realtime, ask_realtime, bid, ask, date, time, day_range, wk_range) VALUES ";
...
while($data = fgetcsv($handle, 4096, ',')){
     ...
     $stocks[$i]['day_range']=$data[9];
     $stocks[$i]['wk_range']=$data[10];
     $i++;
...
$connexion->exec("TRUNCATE TABLE forex_temp");
$connexion->beginTransaction();
for($i=0;$i<count($stocks);$i++){
$sql.="(:symbol".$i.", :name".$i.", :last_trade".$i.", :bid_realtime".$i.", :ask_realtime".$i.", :bid".$i.", :ask".$i.", :date".$i.", :time".$i.", :day_range".$i.", :wk_range".$i."), ";
}
$sql=substr($sql,0,-2);
...
$req->bindValue("day_range".$i,$line['day_range']);
$req->bindValue("wk_range".$i,$line['wk_range']);
}
